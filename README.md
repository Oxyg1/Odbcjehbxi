...
либо произвольный диапазон/точное значение в `/generate`, например `/generate tech 6-9 5`
или `/generate mythic 5 20` — ровно 5 символов, 20 штук).

В выдачу попадают **только свободные** username — занятые и продающиеся на Fragment
отбрасываются автоматически. Бот не сдаётся после первой пачки: если свободных
нашлось меньше, чем запрошено, он генерирует и проверяет ещё, пока не наберёт нужное
количество. Останавливается раньше только если для этого стиля/длины физически
закончились ещё не проверенные варианты (актуально для коротких длин вроде 5
символов — там большинство благозвучных вариантов уже занято).
